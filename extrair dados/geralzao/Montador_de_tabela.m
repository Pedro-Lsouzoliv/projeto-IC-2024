...
%provavelmente por conta dos fileparts
...
for i = 1:length(arquivos)
    ...
    %a partir da separacao, pega apenas o id e a tarefa e junta no nome
    if length(partes) == 6 %esse cenario foi um especifico do sujeito 2217
...
fprintf('Buscando arquivos de grau medio...');
for i=1:length(arquivos)
    caminho = fullfile(diretorio, arquivos(i).name);
    dados = readtable(caminho, 'Delimiter','\t');
    coluna2 = dados{:,2};
...
fprintf('Buscando arquivos de PathLength...');
for i=1:length(arquivos)
    caminho = fullfile(diretorio, arquivos(i).name);
    dados = readtable(caminho, 'Delimiter','\t');
    coluna2 = dados{:,2};%!!
...
writetable(tabela, 'Tabelastat_digit_Ama.xlsx');

%validacao so eh necessaria se os sujeitos tivessem com todas as epocas
%n = length(Sujeitos);
